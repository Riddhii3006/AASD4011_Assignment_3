...
def test_fit_and_predict_regression_model_1d():
    X, y = get_train_data(dim=1)
    model, loss = fit_regression_model(X, y)
    X_test = normalize(torch.tensor([[20.], [15.], [10.]]))
    y_pred = model(X_test)
    assert ((y_pred - normalize(torch.tensor([[1252.3008],
                                              [939.9971],
...
def test_fit_and_predict_regression_model_2d():
    X, y = get_train_data(dim=2)
    model, loss = fit_regression_model(X, y)
    X_test = normalize(torch.tensor([[20., 2.], [15., 3.], [10., 4.]]))
    y_pred = model(X_test)

    assert ((y_pred - normalize(torch.tensor([[1191.9037],
                                              [943.9369],
